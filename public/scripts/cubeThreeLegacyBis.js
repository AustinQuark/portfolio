...
const camera = new THREE.PerspectiveCamera(65, cubeContainer.offsetWidth / cubeContainer.offsetHeight, 1, 2000);
...
readTextFile("scripts/cubeContent.json", function (text) {
	...
	for (var i = 0; i < cubeContent.length; i++) {
		...
		face.src = `images/screenshot${i}.png`;
		face.style.imageRendering = "crisp-edges";
		face.style.zIndex = "1";
		var link = document.createElement("a");
		link.classList.add("linkCube");
		link.href = "#";
		var object = document.createElement("object");
		object.style = "width:75%; height:75%; top:50%; left:50%; transform:translate(-50%, -50%); position:absolute;filter: invert(17%) sepia(14%) saturate(1039%) hue-rotate(144deg) brightness(91%) contrast(87%);";
		object.data = "images/link-external.svg";
		object.type = "image/svg+xml";
		object.style.pointerEvents = "none";

		link.appendChild(object);
		faceElem.appendChild(link);
		faceElem.appendChild(face);
		cube.add(label);
/*
...
function animate() {
	...
	TWEEN.update();
	new Resizer(cubeContainer, camera, renderer, faceRenderer);
